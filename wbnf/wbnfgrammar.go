...
type Node struct{ ast.Node }

func (Node) isWalkableType() {}

func (c Node) OneGrammar() *GrammarNode {
	if child := ast.First(c.Node, "grammar"); child != nil {
		return &GrammarNode{child}
	}
	return nil
}
...
type WalkerOps struct {
	...
	EnterNode                 func(Node) Stopper
	ExitNode                  func(Node) Stopper
...
func (w WalkerOps) WalkNode(node Node) Stopper {
	if fn := w.EnterNode; fn != nil {
		if s := fn(node); s != nil {
			if s.ExitNode() {
				return nil
			} else if s.Abort() {
	...
	if child := node.OneGrammar(); child != nil {
		child := *child
